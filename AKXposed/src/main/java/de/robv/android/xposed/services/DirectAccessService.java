...
public final class DirectAccessService extends BaseService {
	...
	@Override
	public FileResult readFile(String filename, long previousSize,
	...
	public FileResult getFileInputStream(String filename, long previousSize,
		...
		if (previousSize == size && previousTime == time)
			return new FileResult(size, time);
		return new FileResult(new BufferedInputStream(new FileInputStream(
				filename), 16 * 1024), size, time);
	}
}
