...
/**
 * Helpers that simplify hooking and calling methods/constructors, getting and
 * settings fields, ...
 */
public final class XposedHelpers {
	 ...
	 *
	 * <p>
	 * There are various allowed syntaxes for the class name, but it's
	 ...
	 *
	 * @param className
	 *            The class name in one of the formats mentioned above.
	 * @param classLoader
	 *            The class loader, or {@code null} for the boot class loader.
	...
	public static Class<?> findClass(String className, ClassLoader classLoader) {
		...
		try {
			...
			throw new ClassNotFoundError(t == null ? e : t);
		}
	}

	/**
	 * Look up and return a class if it exists. Like {@link #findClass}, but
	 * doesn't throw an exception if the class doesn't exist.
	 *
	 * @param className
	 *            The class name.
	 * @param classLoader
	 ...
	 *
	 * @param clazz
	 *            The class which either declares or inherits the field.
	 * @param fieldName
	 *            The field name.
	...
	/**
	 * Look up and return a field if it exists. Like {@link #findField}, but
	 * doesn't throw an exception if the field doesn't exist.
	 *
	 * @param clazz
	 *            The class which either declares or inherits the field.
	 * @param fieldName
	 ...
	 * Proguard'ed classes to identify fields with unique types.
	 *
	 * @param clazz
	 *            The class which either declares or inherits the field.
	 * @param type
	...
	/**
	 * Look up a method and hook it. See
	 * {@link #findAndHookMethod(String, ClassLoader, String, Object...)} for
	 * details.
	 */
	public static XC_MethodHook.Unhook findAndHookMethod(Class<?> clazz,
		...
		Method m = findMethodExact(
				clazz,
				methodName,
				getParameterClasses(clazz.getClassLoader(),
	...
	/**
	 * Look up a method and hook it. The last argument must be the callback for
	 * the hook.
	 *
	 * <p>
	 * This combines calls to {@link #findMethodExact(Class, String, Object...)}
	 * and {@link XposedBridge#hookMethod}.
	 *
	 * <p class="warning">
	 * The method must be declared or overridden in the given class, inherited
	 * methods are not considered! That's because each method implementation
	 * exists only once in the memory, and when classes inherit it, they just
	 * get another reference to the implementation. Hooking a method therefore
	 * applies to all classes inheriting the same implementation. You have to
	 * expect that the hook applies to subclasses (unless they override the
	 * method), but you shouldn't have to worry about hooks applying to
	 * superclasses, hence this "limitation". There could be undesired or even
	 * dangerous hooks otherwise, e.g. if you hook {@code SomeClass.equals()}
	 * and that class doesn't override the {@code equals()} on some ROMs, making
	 * you hook {@code Object.equals()} instead.
	 *
	 * <p>
	 * There are two ways to specify the parameter types. If you already have a
	 * reference to the {@link Class}, use that. For Android framework classes,
	 * you can often use something like {@code String.class}. If you don't have
	 * the class reference, you can simply use the full class name as a string,
	 * e.g. {@code java.lang.String} or {@code com.example.MyClass}. It will be
	 * passed to {@link #findClass} with the same class loader that is used for
	 * the target method, see its documentation for the allowed notations.
	 *
	 * <p>
	 * Primitive types, such as {@code int}, can be specified using
	 * {@code int.class} (recommended) or {@code Integer.TYPE}. Note that
	 * {@code Integer.class} doesn't refer to {@code int} but to {@code Integer}
	 * , which is a normal class (boxed primitive). Therefore it must not be
	 * used when the method expects an {@code int} parameter - it has to be used
	 * for {@code Integer} parameters though, so check the method signature in
	 * detail.
	 *
	 * <p>
	 * As last argument to this method (after the list of target method
	 * parameters), you need to specify the callback that should be executed
	 * when the method is invoked. It's usually an anonymous subclass of
	 * {@link XC_MethodHook} or {@link XC_MethodReplacement}.
	 *
	 * <p>
	 * <b>Example</b>
	 * 
	 ...
	 *
	 * @param className
	 *            The name of the class which implements the method.
	 * @param classLoader
	 *            The class loader for resolving the target and parameter
	 *            classes.
	 * @param methodName
	 *            The target method name.
	 * @param parameterTypesAndCallback
	 *            The parameter types of the target method, plus the callback.
	 * @throws NoSuchMethodError
	 *             In case the method was not found.
	 * @throws ClassNotFoundError
	 *             In case the target class or one of the parameter types
	...
	/**
	 * Look up a method in a class and set it to accessible. See
	 * {@link #findMethodExact(String, ClassLoader, String, Object...)} for
	 * details.
	 */
	...
	/**
	 * Look up and return a method if it exists. See
	 * {@link #findMethodExactIfExists(String, ClassLoader, String, Object...)}
	 * for details.
	 */
	public static Method findMethodExactIfExists(Class<?> clazz,
	...
	/**
	 * Look up a method in a class and set it to accessible. The method must be
	 * declared or overridden in the given class.
	 *
	 * <p>
	 * See {@link #findAndHookMethod(String, ClassLoader, String, Object...)}
	 * for details about the method and parameter type resolution.
	 *
	 * @param className
	 *            The name of the class which implements the method.
	 * @param classLoader
	...
	public static Method findMethodExact(String className,
			ClassLoader classLoader, String methodName,
			Object... parameterTypes) {
		return findMethodExact(findClass(className, classLoader), methodName,
				getParameterClasses(classLoader, parameterTypes));
	}

	/**
	 * Look up and return a method if it exists. Like
	 * {@link #findMethodExact(String, ClassLoader, String, Object...)}, but
	 * doesn't throw an exception if the method doesn't exist.
	 *
	 * @param className
	 *            The name of the class which implements the method.
	 * @param classLoader
	 ...
	 */
	public static Method findMethodExactIfExists(String className,
			ClassLoader classLoader, String methodName,
			Object... parameterTypes) {
		try {
			return findMethodExact(className, classLoader, methodName,
					parameterTypes);
		} catch (ClassNotFoundError | NoSuchMethodError e) {
			return null;
		}
	 ...
	 *
	 * <p>
	 * This variant requires that you already have reference to all the
	 * parameter types.
	 */
	public static Method findMethodExact(Class<?> clazz, String methodName,
			Class<?>... parameterTypes) {
		String fullMethodName = clazz.getName() + '#' + methodName
				+ getParametersString(parameterTypes) + "#exact";
	...
	/**
	 * Returns an array of all methods declared/overridden in a class with the
	 ...
	 * @param returnType
	 *            The return type, or {@code null} (see above).
	 * @param parameterTypes
	 *            The parameter types.
	 * @return An array with matching methods, all set to accessible already.
	 */
	public static Method[] findMethodsByExactParameters(Class<?> clazz,
	 ...
	 *
	 * <p>
	 * This does'nt only look for exact matches, but for the best match. All
	 * considered candidates must be compatible with the given parameter types,
	 * i.e. the parameters must be assignable to the method's formal parameters.
	 * Inherited methods are considered here.
	 *
	 * @param clazz
	 *            The class which declares, inherits or overrides the method.
	 * @param methodName
	 *            The method name.
	 * @param parameterTypes
	...
	public static Method findMethodBestMatch(Class<?> clazz, String methodName,
			Class<?>... parameterTypes) {
		String fullMethodName = clazz.getName() + '#' + methodName
				+ getParametersString(parameterTypes) + "#bestmatch";
	 ...
	 * This variant determines the parameter types from the classes of the given
	 * objects.
	 */
	public static Method findMethodBestMatch(Class<?> clazz, String methodName,
			Object... args) {
	 ...
	 * See {@link #findMethodBestMatch(Class, String, Class...)} for details.
	 * This variant determines the parameter types from the classes of the given
	 * objects. For any item that is {@code null}, the type is taken from
	 * {@code parameterTypes} instead.
	 */
	public static Method findMethodBestMatch(Class<?> clazz, String methodName,
	...
	private static Class<?>[] getParameterClasses(ClassLoader classLoader,
		...
		for (int i = parameterTypesAndCallback.length - 1; i >= 0; i--) {
			...
			if (type == null)
				throw new ClassNotFoundError("parameter type must not be null",
						null);
	...
	/**
	 * Look up a constructor of a class and set it to accessible. See
	 * {@link #findMethodExact(String, ClassLoader, String, Object...)} for
	 * details.
	 */
	public static Constructor<?> findConstructorExact(Class<?> clazz,
			Object... parameterTypes) {
		return findConstructorExact(clazz,
				getParameterClasses(clazz.getClassLoader(), parameterTypes));
	}

	/**
	 * Look up and return a constructor if it exists. See
	 * {@link #findMethodExactIfExists(String, ClassLoader, String, Object...)}
	 * for details.
	 */
	public static Constructor<?> findConstructorExactIfExists(Class<?> clazz,
			Object... parameterTypes) {
		try {
	 ...
	 * details.
	 */
	public static Constructor<?> findConstructorExact(String className,
			ClassLoader classLoader, Object... parameterTypes) {
		return findConstructorExact(findClass(className, classLoader),
				getParameterClasses(classLoader, parameterTypes));
	}
	 ...
	 * for details.
	 */
	public static Constructor<?> findConstructorExactIfExists(String className,
			ClassLoader classLoader, Object... parameterTypes) {
		try {
	...
	/**
	 * Look up a constructor and hook it. See
	 * {@link #findAndHookMethod(String, ClassLoader, String, Object...)} for
	 * details.
	 */
	public static XC_MethodHook.Unhook findAndHookConstructor(Class<?> clazz,
		...
		XC_MethodHook callback = (XC_MethodHook) parameterTypesAndCallback[parameterTypesAndCallback.length - 1];
		Constructor<?> m = findConstructorExact(
				clazz,
				getParameterClasses(clazz.getClassLoader(),
						parameterTypesAndCallback));
	...
	public static XC_MethodHook.Unhook findAndHookConstructor(String className,
		...
		return findAndHookConstructor(findClass(className, classLoader),
				parameterTypesAndCallback);
	}
	...
	public static Constructor<?> findConstructorBestMatch(Class<?> clazz,
		...
		for (Constructor<?> constructor : constructors) {
			...
			if (ClassUtils.isAssignable(parameterTypes,
					constructor.getParameterTypes(), true)) {
				// get accessible version of method
				if (bestMatch == null
						|| MemberUtils.compareParameterTypes(
	...
	/**
	 * Thrown when a class loader is unable to find a class. Unlike
	 * {@link ClassNotFoundException}, callers are not forced to explicitly
	 * catch this. If uncaught, the error will be passed to the next caller in
	 * the stack.
	 */
	 ...
	 *
	 * @throws NoSuchFieldError
	 *             if there is no parameter with that type.
	 * @hide
	 */
	public static int getFirstParameterIndexByType(Member method, Class<?> type) {
		Class<?>[] classes = (method instanceof Method) ? ((Method) method)
		...
		throw new NoSuchFieldError("No parameter of type " + type
				+ " found in " + method);
	}

	/**
	 * Returns the index of the parameter declared with the given type, ensuring
	 * that there is exactly one such parameter.
	 *
	 * @throws NoSuchFieldError
	 *             if there is no or more than one parameter with that type.
	 * @hide
	 */
			...
			if (classes[i] == type) {
				if (idx == -1) {
					...
					throw new NoSuchFieldError(
							"More than one parameter of type " + type
									+ " found in " + method);
				}
			}
		}
	...
	// #################################################################################################
	/**
	 * Sets the value of an object field in the given object instance. A class
	 * reference is not sufficient! See also {@link #findField}.
	 */
	...
	/**
	 * Sets the value of a {@code boolean} field in the given object instance. A
	 * class reference is not sufficient! See also {@link #findField}.
	 */
	public static void setBooleanField(Object obj, String fieldName,
			boolean value) {
		try {
	...
	/**
	 * Sets the value of a {@code byte} field in the given object instance. A
	 * class reference is not sufficient! See also {@link #findField}.
	 */
	...
	/**
	 * Sets the value of a {@code char} field in the given object instance. A
	 * class reference is not sufficient! See also {@link #findField}.
	 */
	...
	/**
	 * Sets the value of a {@code double} field in the given object instance. A
	 * class reference is not sufficient! See also {@link #findField}.
	 */
	...
	/**
	 * Sets the value of a {@code float} field in the given object instance. A
	 * class reference is not sufficient! See also {@link #findField}.
	 */
	...
	/**
	 * Sets the value of an {@code int} field in the given object instance. A
	 * class reference is not sufficient! See also {@link #findField}.
	 */
	...
	/**
	 * Sets the value of a {@code long} field in the given object instance. A
	 * class reference is not sufficient! See also {@link #findField}.
	 */
	...
	/**
	 * Sets the value of a {@code short} field in the given object instance. A
	 * class reference is not sufficient! See also {@link #findField}.
	 */
	...
	// #################################################################################################
	/**
	 * Returns the value of an object field in the given object instance. A
	 * class reference is not sufficient! See also {@link #findField}.
	 */
	...
	/**
	 * Returns the value of a {@code boolean} field in the given object
	 * instance. A class reference is not sufficient! See also
	 * {@link #findField}.
	 */
	...
	/**
	 * Returns the value of a {@code byte} field in the given object instance. A
	 * class reference is not sufficient! See also {@link #findField}.
	 */
	...
	/**
	 * Returns the value of a {@code char} field in the given object instance. A
	 * class reference is not sufficient! See also {@link #findField}.
	 */
	...
	/**
	 * Returns the value of a {@code double} field in the given object instance.
	...
	/**
	 * Returns the value of a {@code float} field in the given object instance.
	 * A class reference is not sufficient! See also {@link #findField}.
	 */
	...
	/**
	 * Returns the value of an {@code int} field in the given object instance. A
	 * class reference is not sufficient! See also {@link #findField}.
	 */
	...
	/**
	 * Returns the value of a {@code long} field in the given object instance. A
	 * class reference is not sufficient! See also {@link #findField}.
	 */
	...
	/**
	 * Returns the value of a {@code short} field in the given object instance.
	 * A class reference is not sufficient! See also {@link #findField}.
	 */
	...
	// #################################################################################################
	/**
	 * Sets the value of a static object field in the given class. See also
	 * {@link #findField}.
	 */
	...
	/**
	 * Sets the value of a static {@code boolean} field in the given class. See
	 * also {@link #findField}.
	 */
	public static void setStaticBooleanField(Class<?> clazz, String fieldName,
			boolean value) {
		try {
	...
	/**
	 * Sets the value of a static {@code byte} field in the given class. See
	 * also {@link #findField}.
	 */
	public static void setStaticByteField(Class<?> clazz, String fieldName,
			byte value) {
		try {
	...
	/**
	 * Sets the value of a static {@code char} field in the given class. See
	 * also {@link #findField}.
	 */
	public static void setStaticCharField(Class<?> clazz, String fieldName,
			char value) {
		try {
	...
	/**
	 * Sets the value of a static {@code double} field in the given class. See
	 * also {@link #findField}.
	 */
	public static void setStaticDoubleField(Class<?> clazz, String fieldName,
			double value) {
		try {
	...
	/**
	 * Sets the value of a static {@code float} field in the given class. See
	 * also {@link #findField}.
	 */
	public static void setStaticFloatField(Class<?> clazz, String fieldName,
			float value) {
		try {
	...
	/**
	 * Sets the value of a static {@code int} field in the given class. See also
	 * {@link #findField}.
	 */
	public static void setStaticIntField(Class<?> clazz, String fieldName,
			int value) {
		try {
	...
	/**
	 * Sets the value of a static {@code long} field in the given class. See
	 * also {@link #findField}.
	 */
	public static void setStaticLongField(Class<?> clazz, String fieldName,
			long value) {
		try {
	...
	/**
	 * Sets the value of a static {@code short} field in the given class. See
	 * also {@link #findField}.
	 */
	public static void setStaticShortField(Class<?> clazz, String fieldName,
			short value) {
		try {
	...
	// #################################################################################################
	/**
	 * Returns the value of a static object field in the given class. See also
	 * {@link #findField}.
	 */
	...
	/**
	 * Returns the value of a static {@code boolean} field in the given class.
	 * See also {@link #findField}.
	 */
	...
	// #################################################################################################
	/**
	 * Calls an instance or static method of the given object. The method is
	 * resolved using {@link #findMethodBestMatch(Class, String, Object...)}.
	 *
	 * @param obj
	 *            The object instance. A class reference is not sufficient!
	 * @param methodName
	 *            The method name.
	 * @param args
	 ...
	 * @throws NoSuchMethodError
	 *             In case no suitable method was found.
	 * @throws InvocationTargetError
	 *             In case an exception was thrown by the invoked method.
	 */
	public static Object callMethod(Object obj, String methodName,
			Object... args) {
		try {
			return findMethodBestMatch(obj.getClass(), methodName, args)
					.invoke(obj, args);
		} catch (IllegalAccessException e) {
			// should not happen
			XposedBridge.log(e);
	...
	/**
	 * Calls an instance or static method of the given object. See
	 * {@link #callMethod(Object, String, Object...)}.
	 *
	 * <p>
	 * This variant allows you to specify parameter types, which can help in
	 * case there are multiple methods with the same name, especially if you
	 * call it with {@code null} parameters.
	 */
	public static Object callMethod(Object obj, String methodName,
			Class<?>[] parameterTypes, Object... args) {
		try {
			return findMethodBestMatch(obj.getClass(), methodName,
					parameterTypes, args).invoke(obj, args);
		} catch (IllegalAccessException e) {
			// should not happen
			XposedBridge.log(e);
			throw new IllegalAccessError(e.getMessage());
	...
	/**
	 * Calls a static method of the given class. The method is resolved using
	 * {@link #findMethodBestMatch(Class, String, Object...)}.
	 *
	 * @param clazz
	 *            The class reference.
	 * @param methodName
	...
	public static Object callStaticMethod(Class<?> clazz, String methodName,
			Object... args) {
		try {
			return findMethodBestMatch(clazz, methodName, args).invoke(null,
					args);
		} catch (IllegalAccessException e) {
			// should not happen
			XposedBridge.log(e);
	...
	/**
	 * Calls a static method of the given class. See
	 * {@link #callStaticMethod(Class, String, Object...)}.
	 *
	 * <p>
	 * This variant allows you to specify parameter types, which can help in
	 * case there are multiple methods with the same name, especially if you
	...
	/**
	 * This class provides a wrapper for an exception thrown by a method
	 * invocation.
	 *
	...
	// #################################################################################################
	/**
	 * Creates a new instance of the given class. The constructor is resolved
	 * using {@link #findConstructorBestMatch(Class, Object...)}.
	 *
	 * @param clazz
	 *            The class reference.
	 * @param args
	 ...
	 * @throws NoSuchMethodError
	 *             In case no suitable constructor was found.
	 * @throws InvocationTargetError
	 *             In case an exception was thrown by the invoked method.
	 * @throws InstantiationError
	 *             In case the class cannot be instantiated.
	 */
	...
	/**
	 * Creates a new instance of the given class. See
	 * {@link #newInstance(Class, Object...)}.
	 *
	 * <p>
	 * This variant allows you to specify parameter types, which can help in
	 * case there are multiple constructors with the same name, especially if
	 * you call it with {@code null} parameters.
	 */
	...
	/**
	 * Attaches any value to an object instance. This simulates adding an
	 * instance field. The value can be retrieved again with
	 * {@link #getAdditionalInstanceField}.
	 *
	 * @param obj
	 *            The object instance for which the value should be stored.
	 * @param key
	 *            The key in the value map for this object instance.
	 * @param value
	 *            The value to store.
	 * @return The previously stored value for this instance/key combination, or
	 *         {@code null} if there was none.
	 */
	public static Object setAdditionalInstanceField(Object obj, String key,
			Object value) {
		if (obj == null)
	 ...
	 *
	 * @param obj
	 *            The object instance for which the value has been stored.
	 * @param key
	 *            The key in the value map for this object instance.
	 * @return The stored value for this instance/key combination, or
	 ...
	 * {@link #setAdditionalInstanceField}.
	 *
	 * @param obj
	 *            The object instance for which the value has been stored.
	 * @param key
	...
	/**
	 * Like {@link #setAdditionalInstanceField}, but the value is stored for the
	 * class of {@code obj}.
	 */
	public static Object setAdditionalStaticField(Object obj, String key,
			Object value) {
		return setAdditionalInstanceField(obj.getClass(), key, value);
	}

	/**
	 * Like {@link #getAdditionalInstanceField}, but the value is returned for
	 * the class of {@code obj}.
	 */
	...
	/**
	 * Like {@link #removeAdditionalInstanceField}, but the value is removed and
	 * returned for the class of {@code obj}.
	 */
	...
	/**
	 * Like {@link #setAdditionalInstanceField}, but the value is stored for
	 * {@code clazz}.
	 */
	public static Object setAdditionalStaticField(Class<?> clazz, String key,
			Object value) {
		return setAdditionalInstanceField(clazz, key, value);
	}

	/**
	 * Like {@link #setAdditionalInstanceField}, but the value is returned for
	 * {@code clazz}.
	 */
	...
	/**
	 * Like {@link #setAdditionalInstanceField}, but the value is removed and
	 * returned for {@code clazz}.
	 */
	 ...
	 *
	 * @param res
	 *            The resources from which the asset should be loaded.
	 * @param path
	 *            The path to the asset, as in {@link AssetManager#open}.
	...
	/* package */static byte[] inputStreamToByteArray(InputStream is)
			throws IOException {
	 ...
	 *
	 * <p>
	 * The intention of the method depth counter is to keep track of the call
	 * depth for recursive methods, e.g. to override parameters only for the
	 * outer call. The Xposed framework uses this to load drawable replacements
	 * only once per call, even when multiple {@link Resources#getDrawable}
	 * variants call each other.
	 *
	 * @param method
	 *            The method name. Should be prefixed with a unique,
	...
	/**
	 * Decrements the depth counter for the given method. See
	 * {@link #incrementMethodDepth} for details.
	 *
	 * @param method
	 *            The method name. Should be prefixed with a unique,
	 *            module-specific string.
	...
	/**
	 * Returns the current depth counter for the given method. See
	 * {@link #incrementMethodDepth} for details.
	 *
	 * @param method
	 *            The method name. Should be prefixed with a unique,
	...
	private static ThreadLocal<AtomicInteger> getMethodDepthCounter(
			String method) {
	...
	/* package */static boolean fileContains(File file, String str)
			throws IOException {
		// There are certainly more efficient algorithms (e.g. Boyer-Moore used
		// in grep),
	...
	/**
	 * Returns the method that is overridden by the given method. It returns
	 * {@code null} if the method doesn't override another method or if that
	 * method is abstract, i.e. if this is the first implementation in the
	 * hierarchy.
	 */
